            ...
            for i in range(logged):
                ...
                for p in range(logged):
                    if self.adjacency_matrix[p][i] == 1:
                        #p is a prereq for i
                        if self.vertices[p].label not in finished or self.vertices[p].label in semlst:
...
def main():
    ...
    num_vertices = int(sys.stdin.readline().rstrip('\n'))
    ...
    for _ in range(num_edges):
        ...
        line = line.split()
        u = graph.get_index(line[0])
        v = graph.get_index(line[1])
        graph.add_edge(u, v)
